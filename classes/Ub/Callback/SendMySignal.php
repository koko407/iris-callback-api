...
class UbCallbackSendMySignal implements UbCallbackAction {
	...
	function execute($userId, $object, $userbot, $message) {
		...
		if (preg_match('#^(\-смс|дд|рд)([0-9\ ]{1,4})?#', $in, $c)) {
				...
				if ($mid) {
				$vk->messagesEdit(UbVkApi::chat2PeerId($chatId), $mid, UB_ICON_SUCCESS_OFF . " удаляю сообщения ..."); sleep(0.4); }
				$GetHistory = $vk->messagesGetHistory(UbVkApi::chat2PeerId($chatId), 1, 200); sleep(0.4);
				...
				$res = $vk->messagesDelete($ids, true); sleep(0.4);
				if ($mid) {
